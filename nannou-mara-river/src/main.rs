/*
This simulates the crossing of zebras and wildebeest across the Mara River.

Each animal is represented by a simple Boid brain, where the animal has a
"crossing" readiness
...
fn is_point_in_river(point: &Point2, river: &River) -> bool {
    let mut is_in_river = false;
    for (i, width) in river.widths.iter().enumerate() {
        let center = river.centers[i];
